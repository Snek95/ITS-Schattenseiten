...
void loop(){
  ...
  if((abs(a.acceleration.x/xmem)-1)>0.4){   //Schwellenwert (Prozentwert) anpassen // (so  hochstellen, dass bei keiner Bewegung keine 1 mehr in TD ankommt)
    motion+=1;                                //Wenn alter und neuer wert sich um eine bestimmte größe unterscheiden dann ist es eine bewegung
    //Serial.println((a.acceleration.x/xmem)-1);
    //Serial.println("");
  }
  if((abs(a.acceleration.y/ymem)-1)>0.4){   // Wenn Ergebnis von "aktueller Wert durch den Grundwert minus 1" einen Wert höher als den eingestellten Grenzwert hat, dann wird eine 1 generiert
    motion+=1;
    //Serial.println((a.acceleration.y/ymem)-1);
    //Serial.println("");
  }
  if((abs(a.acceleration.z/zmem)-1)>0.4){   //jede Achse wird hier mit einbezogen
    motion+=1;
  ...
  motion=0;
  sensorwert=0;
  xmem=a.acceleration.x;              //Drift ausgleichen indem xmem auf den aktuellen Achsenwert gesetzt wird
  ymem=a.acceleration.y;
  zmem=a.acceleration.z;
  xrot=g.gyro.x;
  yrot=g.gyro.y;
  zrot=g.gyro.z;
  delay(10);                        //bringt auch stabilität ins audiosignal da wert nicht so oft neu verglichen wird
}
