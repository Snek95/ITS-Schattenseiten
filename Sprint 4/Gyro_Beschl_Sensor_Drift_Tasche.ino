...
void setup() {
  ...
  yrot=g.gyro.y;                  //Rotationswerte mit einbeziehen!
  zrot=g.gyro.z;                  //Rotationswerte mit einbeziehen!
}
...
void loop(){
  ...
  if((abs(a.acceleration.x/xmem)-1)>0.4){   //Schwellenwert 40%
    motion+=1;                                
    //Serial.println((a.acceleration.x/xmem)-1);
    //Serial.println("");
  }
  if((abs(a.acceleration.y/ymem)-1)>0.4){   //Schwellenwert 40%
    motion+=1;
    //Serial.println((a.acceleration.y/ymem)-1);
    //Serial.println("");
  }
  if((abs(a.acceleration.z/zmem)-1)>0.4){   //Schwellenwert 40%
    motion+=1;
